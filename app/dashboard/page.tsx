...
export default function UserDashboard() {
  ...
  const [data, setData] = useState<{
    ...
    recentActivities: Array<{ id: string; item: string; gambar?: string; date: string; status: string }>;
  ...
  const handleBorrow = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast(result.error || t("request_failed"), "error");
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-1000">
      ...
      <div className="bg-(--card) rounded-[40px] border border-(--border) p-8 md:p-12 shadow-xl shadow-blue-500/5 relative overflow-hidden group">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8 text-center md:text-left">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-blue-500/10 text-blue-500 rounded-full text-xs font-black uppercase tracking-widest border border-blue-500/10">
              <Sparkles size={14} /> {t('welcome')}
            </div>
            <h1 className="text-4xl md:text-5xl font-black text-(--text-primary) leading-tight">
              {t('lend_tools_desc')} <br /> <span className="text-transparent bg-clip-text bg-linear-to-r from-blue-500 to-pink-500">{t('confidence')}</span>
            </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Recent Activity */}
        <div className="lg:col-span-2 bg-(--card) rounded-[40px] border border-(--border) shadow-xl shadow-blue-500/5 p-8 flex flex-col">
          <div className="flex justify-between items-center mb-10">
            <div className="space-y-1">
              <h3 className="font-black text-2xl text-(--text-primary) uppercase tracking-tight">{t('recent_activity')}</h3>
              <p className="text-xs text-slate-400 font-bold tracking-widest uppercase">{t('history_overview')}</p>
            </div>
            <Link href="/dashboard/peminjaman" className="p-3 bg-blue-500/10 text-blue-500 rounded-2xl hover:bg-blue-500 hover:text-white transition-all group">
          ...
          <div className="flex-1 space-y-4">
            {data.recentActivities.map((row, idx: number) => (
              <div key={idx} className="flex items-center justify-between p-5 bg-(--background)/50 hover:bg-(--card) rounded-3xl border border-transparent hover:border-blue-500/10 transition-all group shadow-sm hover:shadow-md">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-(--card) rounded-2xl flex items-center justify-center text-blue-500 font-black border border-(--border) shadow-sm group-hover:scale-110 transition-transform overflow-hidden">
                    {row.gambar ? (
                      <img src={row.gambar} alt={row.item} className="w-full h-full object-cover" />
                    ) : (
                      row.item.charAt(0)
                    )}
                  </div>
                  <div>
                    <p className="font-bold text-(--text-primary) group-hover:text-blue-500 transition-colors uppercase text-sm tracking-tight">{translateName(row.item)}</p>
                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1 italic">
                      {new Date(row.date).toLocaleDateString(language === "jp" ? "ja-JP" : language === "en" ? "en-US" : language, {
                        month: "short",
                        day: "numeric",
                        year: "numeric"
                      })}
        ...
        <div className="bg-(--card) rounded-[40px] border border-(--border) shadow-xl shadow-blue-500/5 p-8 relative overflow-hidden">
          <div className="flex justify-between items-center mb-8 relative z-10">
            <h3 className="font-black text-xl text-(--text-primary) uppercase tracking-tighter">{t('available_tools')}</h3>
          ...
          <div className="space-y-4 relative z-10">
            {data.recommendedTools.map((tool) => (
              <div
                ...
                className="block p-5 bg-(--background)/50 hover:bg-blue-500/5 rounded-3xl border border-transparent hover:border-blue-500/10 transition-all group cursor-pointer"
              >
                <div className="flex justify-between items-start mb-3">
                  <h4 className="font-black text-(--text-primary) group-hover:text-blue-500 transition-colors text-sm uppercase tracking-tight">{translateName(tool.nama_alat || tool.name)}</h4>
                ...
                <p className="text-[10px] text-slate-400 font-medium line-clamp-2 leading-relaxed mb-4">{translateDesc(tool.deskripsi || "", tool.nama_alat || tool.name || "") || t('premium_equipment_desc')}</p>
...
function DashboardStat({ title, value, icon, color, href }: {
  ...
  return (
    <Link href={href} className={`p-8 rounded-[40px] border shadow-xl shadow-blue-500/5 bg-(--card) relative overflow-hidden group hover:-translate-y-1 transition-all duration-500 flex flex-col justify-between
      ${isBlue ? 'border-blue-500/20' : 'border-pink-500/20'}`}>
      ...
      <div className="relative z-10">
        <p className="text-[10px] font-black text-(--text-muted) uppercase tracking-widest mb-2">{title}</p>
        <h4 className="text-4xl font-black text-(--text-primary) group-hover:text-blue-500 transition-colors tracking-tighter">{value}</h4>
      </div>
    </Link>
  );
}
